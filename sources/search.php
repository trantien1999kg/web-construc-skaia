<?php 
    ...
    $limit = ' limit '.$startpoint.','.$per_page;
  	...
  	$sql = "SELECT * from #_baiviet where type in ('dich-vu','du-an') and hienthi=1 $where $order_by ".$limit;
  	
    $tintuc = $db->rawQuery($sql);

    $sqlq = "SELECT COUNT(*) as `numb` from #_baiviet where type in ('dich-vu','du-an') and hienthi=1 $where $order_by";
