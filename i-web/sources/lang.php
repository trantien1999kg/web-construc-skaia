...
function get_langs(){
	...
	if($_REQUEST['keyword']!=''){
		...
		$where.=" and item like '%$ten%' or lang_vi like '%$ten%' or lang_en like '%$ten%' or lang_jp like '%$ten%'";
...
function delete_lang(){	
	...
	if(isset($_GET['id'])){
		...
		if($db->rawQuery($sql)){
			...
			$func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=ngonngu&act=man_lang".$page);
